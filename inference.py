    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for old_position in self.legalPositions:
            ...
            new_position_distance = self.getPositionDistribution(self.setGhostPosition(gameState, old_position))

            for newPos, prob in new_position_distance.items():
                allPossible[newPos] += self.beliefs[old_position] * prob
...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
            ...
            for particle in self.particles.keys():
                self.particles[particle] = 0
            self.particles[self.getJailPosition()] = 1
            ...
            for position in self.legalPositions:
                true_distance = util.manhattanDistance(position, pacmanPosition)

                if (emissionModel[true_distance] > 0):
                    all_possible[position] = emissionModel[true_distance] * self.particles[position]

            all_possible.normalize()
            self.particles = all_possible

            if all_possible.totalCount() == 0:
                self.initializeUniformly(gameState)
    ...
    def elapseTime(self, gameState):
        ...
        for old_position in self.legalPositions:
            ...
            new_pos_distance = self.getPositionDistribution(self.setGhostPosition(gameState, old_position))

            for new_position, probability in new_pos_distance.items():
                all_possible[new_position] += self.particles[old_position] * probability
...
class JointParticleFilter:
    ...
    def observeState(self, gameState):
        ...
        for particle in self.particles:
            particle_weight = 1.0
            ghost_position = list(particle)

            for ghost_index in range(self.numGhosts):
                distance = util.manhattanDistance(ghost_position[ghost_index], pacmanPosition)

                if noisyDistances[ghost_index] is None:
                    particle = self.getParticleWithGhostInJail(particle, ghost_index)

                else:
                    particle_weight *= emissionModels[ghost_index][distance]

            particle_weights[particle] += particle_weight

        particle_weights.normalize()

        if particle_weights.totalCount() == 0:
            self.initializeParticles()
            ...
            for ghost_index in range(self.numGhosts):
                if noisyDistances[ghost_index] is None:
                    for particle in range(self.numParticles):
                        particles[particle] = self.getParticleWithGhostInJail(self.particles[particle], ghost_index)

        else:
            for _ in range(self.numParticles):
                particles.append(util.sample(particle_weights))
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            for ghost_index in range(self.numGhosts):
                newParticle[ghost_index] = util.sample(getPositionDistributionForGhost(
                    setGhostPositions(gameState, list(oldParticle)), ghost_index, self.ghostAgents[ghost_index]))
